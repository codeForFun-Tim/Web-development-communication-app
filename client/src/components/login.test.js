const {
  ...
  async function loginOK() {
    ...
    await driver.findElement(By.className('pass')).sendKeys('12345', Key.RETURN);
    driver.wait(until.urlIs('http://localhost:3000'), 200);
  }
  
  async function loginFailure() {
    await driver.findElement(By.className('userName')).sendKeys('timqi@seas.upenn.edu');
    await driver.findElement(By.className('pass')).sendKeys('', Key.RETURN);
  }
  
  async function loginNoPass() {
    await driver.findElement(By.className('userName')).sendKeys('timqi@seas.upenn.edu', Key.RETURN);
  }
  ...
  it('login failed', async () => {
    ...
    await driver.findElement(By.className('LoginStatus')).getText().then((text) => {
